...
title: "プルリクエスト（PR）"
quiz:
  - question: "プルリクエスト(PR)の主な目的はどれですか?"
    options:
      - "ブランチを削除する"
      - "変更を別のブランチに取り込んでもらうためのリクエスト"
      - "リポジトリをバックアップする"
      - "コミット履歴を削除する"
    answer: 1
  - question: "PR説明文で'Closes #123'と書くとどうなりますか?"
    options:
      - "PR番号が123になる"
      - "PRがマージされるとIssue #123が自動的にクローズされる"
      - "123個のファイルが変更される"
      - "123人にレビュー通知が送られる"
    answer: 1
  - question: "Squash and mergeの特徴はどれですか?"
    options:
      - "すべてのコミット履歴が残る"
      - "複数のコミットが1つにまとまる"
      - "マージコミットが作成されない"
