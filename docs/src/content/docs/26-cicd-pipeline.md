...
title: "CI/CDパイプライン"
quiz:
  - question: "CIとCDの意味として正しいものはどれですか？"
    options:
      - "CI=コード統合、CD=コードデリバリー"
      - "CI=継続的インテグレーション、CD=継続的デリバリー/デプロイ"
      - "CI=コードインスペクション、CD=コードドキュメント"
      - "CI=カスタムインテグレーション、CD=カスタムデプロイ"
    answer: 1
  - question: "GitHub PagesへのデプロイにGitHub Actionsを使用する利点は何ですか？"
    options:
      - "手動でファイルをアップロードする必要がなくなる"
      - "コードが自動的に修正される"
      - "セキュリティが自動的に強化される"
      - "料金が無料になる"
    answer: 0
  - question: "デプロイワークフローでenvironmentを設定する主な理由は何ですか？"
    options:
      - "ファイルを圧縮するため"
      - "環境固有の変数やシークレット、承認フローを管理するため"
      - "コードをコンパイルするため"
      - "ログを削除するため"
    answer: 1
---
